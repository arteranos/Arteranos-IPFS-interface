...
{
    [TestFixture]
    public class DeserializePrivateKeyTest
    {
        private void test_key(string serializedKey, KeyType expected, string expectedPublicKey)
        {
            ...
            byte[] data2 = SimpleBase.Base16.Decode(expectedPublicKey);
            PublicKey expectedPk = PublicKey.Deserialize(data2);

            KeyPair kp = KeyPair.Import(key);
            Assert.AreEqual(expectedPk, kp.PublicKey);
        }
